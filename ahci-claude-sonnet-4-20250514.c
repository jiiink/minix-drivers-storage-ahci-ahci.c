	...
	u8_t packet[ATAPI_PACKET_SIZE], size_t size, int write)
{
	...
	if (!ps || !packet || size > AHCI_TMP_SIZE) {
		return -1;
	}
...
static int atapi_read_capacity(struct port_state *ps, int cmd)
{
	...
	lba_raw = ((u32_t)buf[0] << 24) | ((u32_t)buf[1] << 16) |
		  ((u32_t)buf[2] << 8) | (u32_t)buf[3];
	sector_size_raw = ((u32_t)buf[4] << 24) | ((u32_t)buf[5] << 16) |
			  ((u32_t)buf[6] << 8) | (u32_t)buf[7];

	if (lba_raw == 0xFFFFFFFF) {
		return EINVAL;
	}

	ps->lba_count = (u64_t)lba_raw + 1;
	ps->sector_size = sector_size_raw;
...
static int atapi_check_medium(struct port_state *ps, int cmd)
{
	...
	result = atapi_test_unit(ps, cmd);
	if (result != OK) {
		ps->flags &= ~FLAG_HAS_MEDIUM;

		result = atapi_request_sense(ps, cmd, &sense);
		if (result != OK || sense != ATAPI_SENSE_UNIT_ATT) {
			return ENXIO;
		}
	}

	if (!(ps->flags & FLAG_HAS_MEDIUM)) {
		result = atapi_read_capacity(ps, cmd);
		if (result != OK) {
			return EIO;
		}
...
static int atapi_id_check(struct port_state *ps, u16_t *buf)
{
    ...
    int is_atapi_removable = (gcap & (ATA_ID_GCAP_ATAPI_MASK | 
                                     ATA_ID_GCAP_REMOVABLE | 
                                     ATA_ID_GCAP_INCOMPLETE)) == 
                            (ATA_ID_GCAP_ATAPI | ATA_ID_GCAP_REMOVABLE);

    int has_dma_support = (cap & ATA_ID_CAP_DMA) == ATA_ID_CAP_DMA;

    int has_dmadir_support = (dmadir & (ATA_ID_DMADIR_DMADIR | ATA_ID_DMADIR_DMA)) == 
                            (ATA_ID_DMADIR_DMADIR | ATA_ID_DMADIR_DMA);

    if (!is_atapi_removable || (!has_dma_support && !has_dmadir_support)) {
    ...
    u16_t device_type = (gcap & ATA_ID_GCAP_TYPE_MASK) >> ATA_ID_GCAP_TYPE_SHIFT;
    if (device_type == ATAPI_TYPE_CDROM) {
        ps->flags |= FLAG_READONLY;
    }

    u16_t sup1 = buf[ATA_ID_SUP1];
    if ((sup1 & ATA_ID_SUP1_VALID_MASK) == ATA_ID_SUP1_VALID &&
        !(ps->flags & FLAG_READONLY)) {
...
static int ata_id_check(struct port_state *ps, u16_t *buf)
{
	...
	if (!is_supported_ata_device(buf)) {
		log_unsupported_device(ps, buf);
		return FALSE;
	}

	extract_lba_count(ps, buf);
	configure_queue_depth(ps, buf);
	
	if (!set_sector_size(ps, buf)) {
		return FALSE;
	}

	set_device_capabilities(ps, buf);

	return TRUE;
}

static int is_supported_ata_device(u16_t *buf)
{
	u16_t gcap_required = ATA_ID_GCAP_ATA;
	u16_t gcap_forbidden = ATA_ID_GCAP_REMOVABLE | ATA_ID_GCAP_INCOMPLETE;
	u16_t cap_required = ATA_ID_CAP_LBA | ATA_ID_CAP_DMA;
	u16_t sup1_required = ATA_ID_SUP1_VALID | ATA_ID_SUP1_FLUSH | ATA_ID_SUP1_LBA48;

	return ((buf[ATA_ID_GCAP] & (ATA_ID_GCAP_ATA_MASK | gcap_forbidden)) == gcap_required) &&
	       ((buf[ATA_ID_CAP] & cap_required) == cap_required) &&
	       ((buf[ATA_ID_SUP1] & (ATA_ID_SUP1_VALID_MASK | ATA_ID_SUP1_FLUSH | ATA_ID_SUP1_LBA48)) == sup1_required);
}

static void log_unsupported_device(struct port_state *ps, u16_t *buf)
{
...
static void extract_lba_count(struct port_state *ps, u16_t *buf)
{
...
static void configure_queue_depth(struct port_state *ps, u16_t *buf)
{
...
static int set_sector_size(struct port_state *ps, u16_t *buf)
{
...
static void set_device_capabilities(struct port_state *ps, u16_t *buf)
{
	...
	u8_t packet[ATAPI_PACKET_SIZE], prd_t *prdt, int nr_prds, int write)
{
	...
	if (!is_fpdma_cmd && (nr_prds > 0 || packet != NULL)) {
		cl[0] |= AHCI_CL_PREFETCHABLE;
	}
...
{
    if (!ps || cmd < 0 || cmd >= ps->queue_depth) {
        return;
    }
...
static void port_check_cmds(struct port_state *ps)
{
    ...
    mask = (ps->flags & FLAG_NCQ_MODE) ? 
           port_read(ps, AHCI_PORT_SACT) : 
           port_read(ps, AHCI_PORT_CI);
	...
	int write, prd_t *prdt)
{
	...
	if (!ps || !iovec || !prdt || nr_req <= 0 || ps->sector_size == 0) {
		return EINVAL;
	}
	...
	r = sys_vumap(endpt, vvec, pcount, 0, write ? VUA_READ : VUA_WRITE,
			&prdt[nr_prds], &pcount);
	if (r != OK) {
...
static void port_override(struct port_state *ps)
{
    ...
    while ((port_read(ps, AHCI_PORT_CMD) & AHCI_PORT_CMD_CLO) && 
        ...
        timeout_count++;
        /* Add delay here based on PORTREG_DELAY */
    }

    if (timeout_count >= max_timeout) {
        dprintf(V_ERR, ("%s: override timeout\n", ahci_portname(ps)));
        return;
    }
...
static void print_string(u16_t *buf, int start, int end)
{
    ...
    for (i = start; i <= end; i++) {
        printf("%c%c", (char)(buf[i] >> 8), (char)(buf[i] & 0xFF));
    }

    if (last) {
        printf("%c", (char)(buf[i] >> 8));
    }
}
...
static void port_handle_disconnect(struct port_state *ps)
{
    port_stop(ps);
...
static void port_dev_check(struct port_state *ps)
{
    ...
    if (status == AHCI_PORT_SSTS_DET_DET) {
        if (ps->left > 0) {
            ps->left--;
            set_timer(&ps->cmd_info[0].timer, ahci_device_delay,
                      port_timeout, BUILD_ARG(ps - port_state, 0));
...
static void port_intr(struct port_state *ps)
{
    ...
    if (emask & AHCI_PORT_IS_PCS) {
        port_handle_device_attach(ps);
    } else if (emask & AHCI_PORT_IS_PRCS) {
        port_handle_device_detach(ps);
    } else if (smask & AHCI_PORT_IS_MASK) {
        port_handle_command_completion(ps);
    }
}

static void port_handle_device_attach(struct port_state *ps)
{
...
static void port_handle_device_detach(struct port_state *ps)
{
...
static void port_handle_command_completion(struct port_state *ps)
{
    u32_t tfd_status;
    int success;
    ...
    if ((tfd_status & (AHCI_PORT_TFD_STS_ERR | AHCI_PORT_TFD_STS_DF)) ||
        (port_read(ps, AHCI_PORT_IS) & AHCI_PORT_IS_RESTART)) {
        port_restart(ps);
    }
...
static void port_timeout(int arg)
{
    ...
    switch (ps->state) {
        case STATE_SPIN_UP:
            handle_spinup_timeout(ps);
            break;
        case STATE_WAIT_DEV:
            port_dev_check(ps);
            break;
        case STATE_WAIT_ID:
            handle_general_timeout(ps);
...
static void handle_spinup_timeout(struct port_state *ps)
{
...
static void handle_general_timeout(struct port_state *ps)
{
...
static void port_issue(struct port_state *ps, int cmd, clock_t timeout)
{
    if (!ps || cmd < 0 || cmd >= MAX_COMMANDS) {
        return;
    }

    if (ps->flags & FLAG_HAS_NCQ) {
        port_write(ps, AHCI_PORT_SACT, 1U << cmd);
    }

    __insn_barrier();

    port_write(ps, AHCI_PORT_CI, 1U << cmd);
...
static int port_exec(struct port_state *ps, int cmd, clock_t timeout)
{
	...
	return (ps->cmd_info[cmd].result == RESULT_FAILURE) ? EIO : OK;
}
...
static void ahci_get_params(void)
{
	...
	timevar_count = sizeof(ahci_timevar) / sizeof(ahci_timevar[0]);
	for (i = 0; i < timevar_count; i++) {
		v = ahci_timevar[i].default_ms;

		(void) env_parse(ahci_timevar[i].name, "d", 0, &v, 1, LONG_MAX);
...
static char *ahci_portname(struct port_state *ps)
{
    ...
    int port_num = (int)(ps - port_state);
    
    if (port_num < 0 || port_num >= 100) {
        return "INVALID";
    }
    ...
    name[2] = 'C';
    name[3] = 'I';
    name[4] = '0' + (ahci_instance % 10);
    name[5] = '-';
    
    if (ps->device == NO_DEVICE) {
        name[6] = 'P';
        name[7] = '0' + (port_num / 10);
        name[8] = '0' + (port_num % 10);
        name[9] = '\0';
    }
    else {
        name[6] = 'D';
        if (ps->device >= 0 && ps->device <= 9) {
            name[7] = '0' + ps->device;
            name[8] = '\0';
            name[9] = '\0';
        } else {
...
static struct device *ahci_part(devminor_t minor)
{
	...
	return ahci_map_minor(minor, &dv);
}
...
static int ahci_handle_first_open(struct port_state *ps)
{
    int r;
...
static void ahci_reset_partitions(struct port_state *ps)
{
